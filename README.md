...
3.  **The Engine**: Beneath the hood, Ella uses Google's experimental **Gemini 3 Pro** (Nano Banana) model. It sends the *actual pixel data* of the selected assets alongside the prompt to the AI, ensuring the final result respects the visual identity of the inputs.
4.  **Ella (The Chatbot)**: A context-aware AI Creative Director who can "see" the current selection and offer tailored advice or prompts.
...
*   **Multimodal Input**: Images are passed directly to the LLM context window.
*   **Persistent Gallery**: Automatically saves shoots and allows batch downloading.
*   **Context-Aware Chat**: Ella knows what you are working on and sees what you see.
